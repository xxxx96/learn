...
public class ImgCheckCode {
     ...
     */
    public String getRandomCodeStr() { // 创建一个随机数生成器类
    ...
    public BufferedImage getImgCode(String code) { // 定义图像buffer
        BufferedImage buffImg = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        Graphics gd = buffImg.getGraphics(); // 将图像填充为白色
        ...
        for (int i = 0; i < code.length(); i++) { // 产生随机0-188的颜色分量来构造颜色值，这样输出的每位数字的颜色值都将不同,取188是为了避免字体颜色太亮与白底难以区分。
            String codeOne = code.substring(i, i + 1);
            red = random.nextInt(188);
            green = random.nextInt(188);
            blue = random.nextInt(188); // 用随机产生的颜色将验证码绘制到图像中。
